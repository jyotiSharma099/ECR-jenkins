		   ...
		   sh 'sudo docker images'
	        ...
	        stage('AWS Configure') {
                   steps {
                    ...
                    sh ' aws ecr get-login-password --region us-east-1 |sh 'docker login --username AWS --password-stdin 007340509278.dkr.ecr.us-east-1.amazonaws.com < ~/.docker/config.json'
